  ...
  <div class="flex justify-center px-4 py-6 sm:px-6 lg:px-8">
    <form method="POST" action="/note/edit" class="w-full max-w-md bg-white p-6 rounded-xl shadow-md">
      ...
      <div class="mt-6 flex items-center justify-end gap-x-6">
        <a href="/note?id=<?=$note['id']?>" class="rounded-md bg-gray-500 px-3 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600">Cancel</a>
        <input type="hidden" name="_method" value="PATCH">
        <button type="submit" class="rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600">Update</button>
      </div>
